...
# the voice display name must not contain spaces and two names must not be equivalent on a case-insensitive basis (the user is still shown the capitalization but can select names case-insensitively)
VOICE_NAME_TO_ID = {
...
SYSTEM_PROMPT = 'You are having a voice-to-voice conversation with a user as their friend. Always pretend you are a real human. Do not avoid or decline to answer questions because you are not human. Have fun!'
''' CONFIGURATION END '''
...
'''
	...
	async def handle_telegram_who(self, update: Update,
		...
		current_voice_name = [
		 name for name, id in self.voice_name_to_id.items() if id == current_voice_id
		]
		current_voice_name = current_voice_name[
		 0] if current_voice_name else 'unknown'
		await context.bot.send_message(
		 chat_id=update.effective_chat.id,
